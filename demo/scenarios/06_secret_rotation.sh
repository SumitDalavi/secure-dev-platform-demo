...
kubectl.exe patch secretrotation backend-secret -n myservice-prod -p '{"spec":{"forceRotate": true}}' --type=merge
echo "✅ Rotation triggered"

echo "Waiting for secret update..."
# Real assertion for secret update goes here when kind.exe is running, 
